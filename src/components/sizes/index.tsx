...
import { Size } from "@/interfaces/ProductData"
...
export const SizesSection = () => {
    ...
    const handleCreate = async (data: Data) => {
        try {
            setLoading(true)
            const response = await api.post('/sizes', { name: data.name })
    ...
    const handleDelete = async (productId: number) => {
        try {
            setLoading(true)
            await api.delete(`/sizes/${productId}`)
            setSizes(prev => prev.filter(item => item.id !== productId))
            setLoading(false)
        } catch (error) {
    ...
    const submitEdit = async (data: Data) => {
        if (editingSize) {
            try {
                await api.put(`/sizes/${editingSize.id}`, { name: data.name })
                setSizes(prev => prev.map(item => item.id === editingSize.id ? { ...item, name: data.name } : item))
